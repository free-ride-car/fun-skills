...
1. 运行 `git branch -a` 获取所有分支列表
2. 从分支列表中识别测试分支：优先匹配名称含 `test`、`testing`、`staging`、`dev`、`develop`、`uat`、`qa` 的分支（本地或远程均可）
3. 如果找到测试分支，询问用户：

   **提交成功！是否将当前分支合并到测试分支？**
   - [1] 合并到 `<测试分支名>` - 执行 `git checkout <测试分支> && git merge <当前分支> && git checkout <原分支>`
   - [2] 跳过 - 不执行合并

4. 如果找到多个测试分支，列出所有候选分支让用户选择
5. 如果未找到任何测试分支，跳过此步骤，不询问
